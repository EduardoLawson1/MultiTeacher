...
def main():
    ...
    model = DPT(**{
        **model_configs[cfg['backbone'].split('_')[-1]],
        'nclass': cfg['nclass'],
        'group_nclass': group_nclass,
    })

    # Carrega backbone pré-treinado
    ...
    if rank == 0:
        ...
        logger.info('Decoder params (heads_multi): {:.1f}M\n'.format(
            sum(count_params(h) for h in model.heads_multi)
        ))
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            with torch.no_grad():
                ...
                mask_u_w, conf_u_w = fuse_teacher_predictions(
                    outs_u_w, group_to_global, cfg['nclass'],
            ...
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info('Iters: {:}, LR: {:.7f}, Total loss: {:.3f}, Loss x: {:.3f}, '
                            'Loss s: {:.3f}, Mask ratio: {:.3f}'.format(
        ...
        if mIoU     == previous_best:     best_epoch     = epoch
        if mIoU_ema == previous_best_ema: best_epoch_ema = epoch
